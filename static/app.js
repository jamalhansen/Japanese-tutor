...
async function fetchDueCards(practice = false) {
  ...
  document.getElementById("due-count").innerText = practice
    ? "Practice Mode"
    : `Due: ${dueCards.length}`;
  ...
  if (dueCards.length > 0) {
    ...
    if (!currentSessionId) {
      ...
      const sessionRes = await fetch("/api/session/start", {
        ...
        body: JSON.stringify({
          stage: practice ? "practice" : dueCards[0].stage,
        }),
      });
...
async function showSessionSummary(trulyAllDone = false) {
  ...
  if (trulyAllDone) {
    title.innerText = isPracticeMode
    ...
    content.innerText = isPracticeMode
      ...
      : "Great job! Keep going to review more cards.";
    keepGoing.classList.remove("hidden");
    keepGoing.innerText = "Keep Learning";
...
function showNextCard() {
  ...
  document.getElementById("mnemonic-display").innerText =
